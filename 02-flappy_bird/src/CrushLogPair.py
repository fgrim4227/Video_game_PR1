...
class CrushLogPair(LogPair):
    def __init__(self, x: float, y: float, log_gap = settings.LOGS_GAP) -> None:
        ...
        self.crush_vy = settings.LOG_CRUSH_SPEED + random.uniform(-0.3, 0.3) * settings.LOG_CRUSH_SPEED  
        self.retreat_vy = settings.LOG_RETREAT_SPEED + random.uniform(-0.2, 0.4) * settings.LOG_RETREAT_SPEED
        self.falling = True
