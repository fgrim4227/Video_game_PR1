    ...
    def generation(self, world, dt, score):
        if world.generate_logs:
            ...
            if world.logs_spawn_timer >= world.current_limit:
                ...
                world.current_limit = random.uniform(settings.LOG_SPAWN_LOWER_LIM, settings.LOG_SPAWN_UPPER_LIM)

                max_dy = 80 * world.current_limit 
                vertical_pair_gap = random.randint(settings.LOG_GAP_LOWER_LIM, settings.LOG_GAP_UPPER_LIM)
                ...
                chance_spawn_logs = 10 + (score // 10)
                if random.randint(1, 100) <= chance_spawn_logs:
                ...
                if random.randint(1, 100) <= settings.CHANCE_POWERUP:
                    pw_y = y + settings.LOG_HEIGHT + (vertical_pair_gap / 2)
