...
class Ball:
    ...
    def unstuck(self, paddle : Any, rebound : bool = False):
        br = self.get_collision_rect()
        sr = paddle.get_collision_rect()
        ...
        if(min_shift ==  abs(shift_x)):
            self.x += shift_x
            self.vx *= -1 if rebound else self.vx
        else:
            self.y += shift_y
            self.vy *= -1 if rebound else self.vy
        return (shift_x, shift_y)
    def rebound(self, another: Any):
        self.unstuck(another, rebound = True)
